...
public class RegisterjPn extends javax.swing.JPanel {
    ...
    public RegisterjPn() {
        ...
        semester=jPNQLRegistrationSession.current;
        if(semester==null){
    ...
    void setBtnRegister(){
        int idcourse=Integer.parseInt(labelId.getText());
        List<RegistrationEntity> ds=RegistrationDAO.listRegistration(namelogin,semester.getSemesterId());
        if(ds!=null){
            int num=ds.size();
            if(num>7){
                JOptionPane.showMessageDialog(null, "Chỉ được đăng ký tối đa 8 học phần!");
                return;
            }
        }
